...
class checkmate_in_n_concurrent_dfs_atomic {
    Position pos;
    int threads_size;
...
public:
    ...
    checkmate_in_n_concurrent_dfs_atomic(Position pos, int threads_size)
    ...
    bool findAnswer(int depth) {
        ...
        for (int i = 0; i < threads.size(); i++) {
            ...
            threads[i] = thread(&checkmate_in_n_concurrent_dfs_atomic::do_work, this, i * (ps.size() / threads_size), ending_index, ps, depth, i);
        }
